...
//QUESTION 2
//SECTION B 
//CANNOT RUN FOR SOME REASON WILL CHECK LATER 
...
class cube
{
	...
	//(1) Class geometry as friend
	public: 
	friend class geometry; 
	
	//(2) Define function cal_vol(...)
	//reference argument g of class geometry 
	void calc_vol(geometry &g)
	{
		//Formula : length power 3
		vol = pow(g.radius, 3);
		cout << "\nVolume of cube with length ";
		cout << g.radius << "is: " << vol << endl;
	}
};

int main ()
{
	//declare object cb of class cube 
	cube cb; 
	float hg, rd;
	cout << "Enter height: ";
	cin >> hg;
	...
	//--12--
	cb.cal_vol(gmt);  //call object cb calc_vol passing in object gmt
